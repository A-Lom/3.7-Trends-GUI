...
class Window(QMainWindow):
    ...
    def button_time_clicked(self):
        ...
        if len(list) > 5:
            ...
            return
        if self.multi_graph_check.isChecked():
            pass
        else:
            plt.close('all')
    ...
    #When people do things that will crash the program these events will happen
    ...
    #Alet for if a user has more than 5 search terms due to google only accepting a max of 5
    def max_num_alert(self, list_num):
        alert = QMessageBox()
        alert.setWindowTitle("ALERT")
        alert.setText(f"You have inputed more than 5 search terms.\nPlease remove {list_num-5} search term(s) and try again")
        alert.setIcon(QMessageBox.Critical)
        alert.setStandardButtons(QMessageBox.Ok)
        alert.exec()
...
def error():
        ...
        alert.setInformativeText("This could be due to there being no graph to be displayed")
        alert.setIcon(QMessageBox.Critical)
        alert.setStandardButtons(QMessageBox.Ok)
        alert.exec()
...
def interest_over_time(list):
    ...
    interest_over_time.plot(y=list, figsize=(120, 10), kind ='line')
    plt.show()
...
def interest_by_region(list, region):
    ...
    try:
        ...
        error()
        return
    interest_by_region.plot(y=list, figsize=(120, 10), kind ='bar')
    plt.show()
